			...
			try {
					...
					if (pacote.message().estaIniciandoOutroJogador() && quantidadeDeJogadores < 2) {
						...
						if (quantidadeDeJogadores == 1) {
							transmissao.transmitirPacote(
									new Pacote(ip, porta, FabricaDeMensagem.criarMensagemDeEsperandoJogador()));
						} else {
							transmissao.transmitirPacote(
									new Pacote(ip, porta, FabricaDeMensagem.criarMensagemJogadoresProntos()));
				...
				case jogoIniciado:
					...
					if (quantidadeDeJogadores == 2) {
						//Imprimir tabuleiro para os jogadores
						for(Jogador jogador : jogadoresMapeados.values()) {
							transmissao.transmitirPacote(new Pacote(InetAddress.getByName(jogador.getIp()), jogador.getPorta(),
						...
						for (Jogador jogador : jogadoresMapeados.values()) {
							...
							if (jogador.getId() == 0) {
								// Mensagem para o jogador que inicia 
								jogador.setSuaVez(true);
								transmissao.transmitirPacote(new Pacote(InetAddress.getByName(jogador.getIp()),
										jogador.getPorta(), FabricaDeMensagem.criarMensagemJogadorInicia()));
								//String tabuleiroVazio = jogo.imprimirTabuleiro();
								jogo.status = StatusJogo.esperandoJogada;
							} else {
								// vez do proximo jogador
								transmissao.transmitirPacote(new Pacote(InetAddress.getByName(jogador.getIp()),
										jogador.getPorta(), FabricaDeMensagem.criarMensagemJogadorEspera()));
								//jogo.status = StatusJogo.esperandoJogada;
							}
						}
					}
				...
				case esperandoJogada:
					...
					for (Jogador jogador : jogadoresMapeados.values()) {
						if(jogador.isSuaVez() == false) {
							...
							Pacote messageAguardeSuaVez = new Pacote(InetAddress.getByName(jogador.getIp()), jogador.getPorta(), mensagem);
							transmissao.transmitirPacote(messageAguardeSuaVez);
						}
						
						
//						transmissao.transmitirPacote(new Pacote(InetAddress.getByName(jogador.getIp()),
//								jogador.getPorta(), FabricaDeMensagem.criarMensagemSuaVez()));
...
//						                        		System.out.println("qualquer coisa aí 3");
						...
						transmissao.transmitirPacote(new Pacote(InetAddress.getByName(jogador.getIp()),
								jogador.getPorta(), FabricaDeMensagem.criarMensagemTabuleiro(tabuleiro)));
					}
